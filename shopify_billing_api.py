...
def activate_billing_charge(shop_domain, charge_id):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            store.billing_on = (
                datetime.fromisoformat(charge.get("billing_on").replace("Z", "+00:00"))
                if charge.get("billing_on")
                else None
            )
            store.trial_ends_at = (
                datetime.fromisoformat(
                    charge.get("trial_ends_on").replace("Z", "+00:00")
                )
                if charge.get("trial_ends_on")
                else None
            )
    ...
    try:
        ...
        if store.is_subscribed and store.subscription_id:
            ...
            if response.status_code == 200:
                ...
                store.billing_on = (
                    datetime.fromisoformat(
                        charge.get("billing_on").replace("Z", "+00:00")
                    )
                    if charge.get("billing_on")
                    else None
                )
                ...
                return {
                    ...
                    "billing_on": store.billing_on.isoformat()
                    if store.billing_on
                    else None,
                }
...
def get_billing_plans():
    ...
    try:
        ...
        return jsonify({"plans": BILLING_PLANS, "current_status": status, "shop": shop})
...
def subscribe_to_plan():
    ...
    try:
        ...
        # Create return URL
        return_url = url_for(
            "shopify_billing.billing_callback",
            shop=shop,
            plan_id=plan_id,
            _external=True,
        )
        ...
        return jsonify({"success": True, "billing_url": billing_url})
