    ...
    try:
        ...
        with app.app_context():
            ...
            for store in invalid_stores:
                print(
                    f"  - {store.shop_url} (User: {store.user.email if store.user else 'Unknown'})"
                )
                print(f"    ⚠️  This is not a valid Shopify store URL!")
                print(
                    f"    SOLUTION: User needs to reconnect with a real Shopify store"
                )
        ...
        with app.app_context():
            ...
            for user in users:
                if not user.has_access():
                    ...
                    if (
                        input(f"\nExtend trial for {user.email}? (y/n): ").lower()
                        == "y"
                    ):
                        user.trial_ends_at = datetime.now(timezone.utc) + timedelta(
                            days=7
                        )
                        from app import db
